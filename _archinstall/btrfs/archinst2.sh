...
CMD="pacman -Syyu grub nano vi vim networkmanager sudo man gpm rsync openssh pacman-contrib pacutils base-devel git reflector btrfs-progs efibootmgr snapper"
read && $CMD
...
CMD="sed --in-place -E '/^MODULES=\(\)/s/\(\)/(btrfs)/g' /etc/mkinitcpio.conf"
read && eval "$CMD"

grep -A3 -B3 '^MODULES' /etc/mkinitcpio.conf
...
CMD="groupadd snapviewer"
read && $CMD

CMD="useradd -m -G wheel,snapviewer $MAINUSER"
read && $CMD
